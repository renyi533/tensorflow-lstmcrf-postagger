...
def crf_filtered_inputs(inputs: tf.Tensor, tag_bitmap: tf.Tensor) -> tf.Tensor:
...
def crf_log_likelihood(
    ...
    tag_indices: tf.Tensor,
    sequence_lengths: tf.Tensor,
    transition_params: Optional[tf.Tensor] = None,
) -> tf.Tensor:
...
def structured_perceptron_potential_diff(
    inputs: tf.Tensor,
    tag_indices: tf.Tensor,
    sequence_lengths: tf.Tensor,
    transition_params: Optional[tf.Tensor] = None,
    ...
    Args:
      ...
      transition_params: A [num_tags, num_tags] transition matrix,
          if available.
    Returns:
      potential_diff: A [batch_size] `Tensor` containing the potential diff of
        each example, given the sequence of true&predicted tag indices.
      transition_params: A [num_tags, num_tags] transition matrix. This is
          either provided by the caller or created in this function.
    """
    inputs = tf.convert_to_tensor(inputs)
    ...
    decode_tags, best_score = crf_decode(inputs, \
        sequence_length=sequence_lengths, \
        transition_params=transition_params)
    decode_sequence_scores = crf_sequence_score(
        inputs, decode_tags, sequence_lengths, transition_params
    )
    # potential diff of true/predicted tag sequences to be maximized
    potential_diff = sequence_scores - decode_sequence_scores
    return potential_diff, transition_params
...
class CrfBackwardRnnCell(rnn_cell.RNNCell):
  """Computes the beta values in a linear-chain CRF.
  See http://www.cs.columbia.edu/~mcollins/fb.pdf for reference.
  """

  def __init__(self, transition_params):
    """Initialize the CrfBackwardRnnCell.
    Args:
      transition_params: A [num_tags, num_tags] matrix of binary potentials.
          This matrix is expanded into a [1, num_tags, num_tags] in preparation
          for the broadcast summation occurring within the cell.
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    Args:
      inputs: A [batch_size, num_tags] matrix of unary potentials.
      state: A [batch_size, num_tags] matrix containing the previous beta 
          values.
      scope: Unused variable scope of this cell.
    Returns:
      new_betas, new_states: A pair of [batch_size, num_tags] matrices
          values containing the new alpha values.
    """
    curr_beta = state
    exp_state = array_ops.expand_dims(state, 1)

    # This addition op broadcasts self._transitions_params along the zeroth
    # dimension and state along the first dimension. This performs the
    # multiplication of previous beta values and the current binary potentials
    # in log space.
    transition_scores = exp_state + self._transition_params
    new_states = inputs + math_ops.reduce_logsumexp(transition_scores, [2])

    return curr_beta, new_states
...
def crf_forward_alpha(
    inputs: tf.Tensor, sequence_lengths: tf.Tensor, transition_params: tf.Tensor
) -> tf.Tensor:
    """Computes the alphas for a CRF.
    ...
    Args:
      ...
      transition_params: A [num_tags, num_tags] transition matrix.
    Returns:
      alphas: A [batch_size, max_seq_len, num_tags] vector containing the
          new alpha values.
    """
    sequence_lengths = tf.cast(sequence_lengths, dtype=tf.int32)
    # Split up the first and rest of the inputs in preparation for the forward
    # algorithm.
    first_input = tf.slice(inputs, [0, 0, 0], [-1, 1, -1])
    cond_length = tf.reshape(sequence_lengths, [-1, 1, 1]) 
    first_input = tf.where(
        tf.less_equal(cond_length, 0), tf.zeros_like(first_input), first_input
    )
    init_state = tf.squeeze(first_input, [1])
    ...
    def _multi_seq_fn():
        ...
        alphas, state = rnn.dynamic_rnn(
            cell=forward_cell,
            inputs=rest_of_input,
            sequence_length=sequence_lengths_less_one,
            initial_state=init_state,
            dtype=dtypes.float32)
        alphas = tf.concat([first_input, alphas], axis=1)

        return alphas

    return tf.cond(tf.equal(tf.shape(inputs)[1], 1), _single_seq_fn, _multi_seq_fn)

def crf_fb_alpha_beta(
    inputs: tf.Tensor, sequence_lengths: tf.Tensor, transition_params: tf.Tensor
) -> Tuple[tf.Tensor, tf.Tensor, tf.Tensor, Optional[tf.Tensor], tf.Tensor]:
    """Computes the alphas/betas for a CRF.
    ...
    Returns:
      alphas: A [batch_size, max_seq_len, num_tags] vector containing the
          new alpha values.
      betas: A [batch_size, max_seq_len, num_tags] vector containing the
          new beta values.
      step_log_likelihood: A [batch_size, max_seq_len, num_tags] vector containing the 
          log_likelihood for tags in each step.
      trans_log_likelihood: A [batch_size, max_seq_len, num_tags, num_tags] vector containing  
          the log_likelihood for state transition in each step.
      log_norm: A [batch_size] vector containing the log_norm for each instance.
    """
    sequence_lengths = tf.cast(sequence_lengths, dtype=tf.int32)
    # Split up the first and rest of the inputs in preparation for the forward
    # algorithm.
    ...
    init_forward_state = tf.squeeze(first_input, [1])
    init_backward_state = tf.zeros_like(init_forward_state)

    # If max_seq_len is 1, we skip the algorithm and simply reduce_logsumexp
    # over the "initial state" (the unary potentials).
    def _single_seq_fn():
        alphas = first_input
        betas = tf.expand_dims(init_backward_state, axis=1)
        log_likelihood, log_norm = _get_step_log_likelihood(alphas, betas)
        return alphas, betas, log_likelihood, None, log_norm
    ...
    def _multi_seq_fn():
        ...
        outputs, output_states = rnn.bidirectional_dynamic_rnn(
            cell_fw=forward_cell,
            cell_bw=backward_cell,
            inputs=rest_of_input,
            sequence_length=sequence_lengths_less_one,
            initial_state_fw=init_forward_state,
            initial_state_bw=init_backward_state,
            dtype=dtypes.float32)
        alphas, betas = outputs
        state_fw, state_bw = output_states
        alphas = tf.concat([first_input, alphas], axis=1)
        first_beta = array_ops.expand_dims(state_bw, 1)
        betas = tf.concat([first_beta, betas], axis=1)
        step_log_likelihood, log_norm = _get_step_log_likelihood(alphas, betas)
        trans_log_likelihood = _get_trans_log_likelihood(alphas, betas, log_norm)
        return alphas, betas, step_log_likelihood, trans_log_likelihood, log_norm
    
    def _get_step_log_likelihood(alphas, betas):
        step_cnt = tf.shape(alphas)[1]
        step_potential = alphas + betas
        log_norms = tf.reduce_logsumexp(step_potential, axis=2, keepdims=True)
        step_log_likelihood = step_potential - log_norms
        log_norm = tf.slice(log_norms, [0, 0, 0], [-1, 1, -1])
        log_norm = tf.reshape(log_norm, [-1])
        log_norm = tf.where(
                tf.less_equal(sequence_lengths, 0), 
                tf.zeros_like(log_norm), 
                log_norm
            )
        step_range = tf.range(0, step_cnt, 1)
        step_range = tf.reshape(step_range, [1, -1, 1])
        cond_length = tf.reshape(sequence_lengths, [-1, 1, 1]) 
        step_log_likelihood = tf.where(
                tf.less(step_range, cond_length), 
                step_log_likelihood,
                tf.zeros_like(step_log_likelihood)
            )
        return step_log_likelihood, log_norm

    def _get_trans_log_likelihood(alphas, betas, log_norm):
        alphas_src = tf.expand_dims(alphas, axis=3)
        beta_dst = tf.expand_dims(betas, axis=2)
        beta_inputs = tf.expand_dims(inputs, axis=2)
        step_cnt = tf.shape(alphas_src)[1]
        alpha_src = tf.slice(alphas_src, begin=[0, 0, 0, 0], \
            size=[-1, step_cnt-1, -1, -1])
        beta_dst = tf.slice(beta_dst, begin=[0, 1, 0, 0], \
            size=[-1, step_cnt-1, -1, -1])
        beta_inputs = tf.slice(beta_inputs, begin=[0, 1, 0, 0], \
            size=[-1, step_cnt-1, -1, -1])
        trans_params = tf.expand_dims(tf.expand_dims(transition_params, 0), 0)
        trans_potential = alpha_src + beta_dst + trans_params + beta_inputs
        log_norms = tf.reshape(log_norm, [-1, 1, 1, 1])
        trans_log_likelihood = trans_potential - log_norms
        step_range = tf.range(0, step_cnt-1, 1)
        step_range = tf.reshape(step_range, [1, -1, 1, 1])
        cond_length = tf.reshape(sequence_lengths, [-1, 1, 1, 1]) 
        trans_log_likelihood = tf.where(
                tf.less(step_range+1, cond_length), 
                trans_log_likelihood,
                tf.zeros_like(trans_log_likelihood)
            )
        return trans_log_likelihood
...
class CrfDecodeForwardRnnCell(tf.keras.layers.AbstractRNNCell):
    ...
    #@typechecked
    def __init__(self, transition_params: tf.Tensor, **kwargs):
...
def crf_decode_backward(inputs: tf.Tensor, state: tf.Tensor) -> tf.Tensor:
